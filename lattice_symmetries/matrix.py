...
class O(LinearOperator):
    ...
    def apply_to_state_vector(self, vector: NDArray, out=None, cast_to=None):
        if np.issubdtype(self.dtype, np.complexfloating):
            vector = vector.astype(self.dtype, copy=False)
        if cast_to is not None:
            dtype, real = vector.dtype, np.empty(0, dtype=vector.dtype).real.dtype
            vector = vector.view(real).astype(cast_to).astype(real).view(dtype)
        r = self._prepare_Matvec()(self.b.states, self.b.norms, vector, out=out)
        if cast_to is not None:
            r = r.view(real).astype(cast_to).astype(real).view(dtype)
        return r
